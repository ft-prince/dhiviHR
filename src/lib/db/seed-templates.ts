...
//   const insertedQuestions: Array<{ id: string; competencyId: string }> = [];
//   for (let i = 0; i < QUESTION_BANK.length; i++) {
//     const q = QUESTION_BANK[i];
//     const existing = await db
//       .select({ id: questions.id, competencyId: questions.competencyId })
//       .from(questions)
...
//           orderIndex: i,
//         })
//         .returning({ id: questions.id, competencyId: questions.competencyId });
...
//       const qs = insertedQuestions.filter((q) => q.competencyId === competencySlug);
//       for (const q of qs) {
