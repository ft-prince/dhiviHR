...
export function QuestionCreatePanel({ competencies }: { competencies?: { id: string; slug: string; label: string }[] }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 p-4">
      <div className="w-full max-w-2xl rounded-2xl border border-border bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
        ...
        <QuestionForm competencies={competencies} onDone={() => setOpen(false)} />
      </div>
    </div>
  );
}
